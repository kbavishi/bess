...
struct CIDRNetwork {
  ...
  bool Match(const be32_t& ip) const {
		// Use raw_value() since these operations are not affected by endianness
    return (addr.raw_value() & mask.raw_value()) ==
           (ip.raw_value() & mask.raw_value());
  }
